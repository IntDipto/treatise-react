...
function MorePosts({allStories}) {
    // console.log(allStories);
    const limitBlog = allStories.splice(0,4)
    // console.log(limitStories);
  return (
    <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-6 gap-5">
        {
            limitBlog?.map(blog =>
                <div key={blog.id} className="border rounded-[10px] "> 
                    <img className="w-full rounded-t-[10px]" src={require(`./../../assests/images/home/best-stories/${blog.img}.webp`)} alt="" srcSet=""/>
                    <div className="p-4">
